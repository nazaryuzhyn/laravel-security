...
Можливо, у вашій заявці є публічна форма для подання файлу.
У цьому випадку великі файли можуть виснажувати пам'ять сервера.
Потрібно мати на увазі, що сервер повинен обслуговувати дані/ресурси іншим користувачам і обробляти цей тип повідомлень
одночасно.

Щоб впоратися з цією атакою, ви можете використовувати засіб перевірки безпеки Laravel API для перевірки файлу із
запиту.
Ось приклад:
...
Laravel надає кілька функцій для запобігання атакам SQL ін'єкції.
...
Однак розробники все ще вибирають необроблений SQL з різних причин.

Якщо у вас це так, ви завжди повинні використовувати добре підготовлені запити SQL, щоб запобігти нещасним випадкам.
Розглянемо наступний оператор, який виглядає готовим для впровадження SQL:
...
Тут оператор 1=1, використаний в умові АБО, призведе до повернення всіх рядків у таблиці користувачів.
Цьому можна запобігти, використовуючи натомість такий код:
...
Laravel замінює знаки питання змінною запиту, автоматично екрануючи вхідні змінні. Це захищає код від атак SQL-ін'єкцій.

### Prevent XSS <a name="prevent-xss"></a>
...
і воно згенерує файл, як показано нижче або просто скопіюйте та вставте цей код у свій middleware файл.
...
Ви можете створити middleware, запустивши `php artisan make:middleware HttpsMiddleware`
і воно згенерує файл, як показано нижче або просто скопіюйте та вставте цей код у свій middleware файл.
...
### [Laravel-ACL](https://github.com/yajra/laravel-acl)

Laravel-ACL надає захищені дозволи на основі ролей для процесу автентифікації Laravel.
Пакет допомагає захистити маршрути та методи контролера CRUD у програмах.
